(ns bip-examples.todos.view
  (:require [bip-examples.todos.bip :as bip]
            [bip-examples.todos.messages :as messages]
            [bip-examples.todos.routes :refer [href-for]]
  ...
  [:div {:id "todoapp"}
   [:header {:class "header"}
    ...
    [:input {:class "new-todo"
             ...
             :on-change (bip/send-value! ui-channel messages/->UpdateNewTodo)
             :auto-focus true}]]
   [:section {:class "main"}
    [:input {:id "toggle-all"
             :class "toggle-all"
    ...
    [:div.col-xs-12.col-sm-8
     [:div.well
      (case (-> state :view :handler)
        ...
        :todos [bip/subcomponent todos ui-channel [:todos] (:todos state)]
        "Home")]]]
